...
const Logistica = () => {
  ...
  const mapOrderFields = (o) => {
    return {
      ...
      transportista_nombre: o.transportista_nombre ?? o.transportista ?? o.transportista_name ?? null,
      patente: o.patente ?? o.patent ?? o.camion_patente ?? o.patent_plate ?? null
    };
  };
  ...
  const kpiViajes = safeArray(ordenes).filter(o => o && (o.estado === "en_carga" || o.estado === "en_ruta")).length;
  ...
  const choferesOrdenados = safeArray(choferes).sort((a, b) => (a.nombre || "").localeCompare(b.nombre || ""));
  const operacionesActivas = safeArray(ordenes).filter(o => o && (o.estado === "en_carga" || o.estado === "en_ruta"));
  ...
  return (
    <div className="logi-container">
      <div className="logi-kpi-container">
        <div className="logi-kpi-card"><h3>{kpiCamiones}</h3><p>Camiones Registrados</p></div>
        <div className="logi-kpi-card"><h3>{kpiChoferes}</h3><p>Choferes Base</p></div>
        <div className="logi-kpi-card bg-blue-50"><h3>{kpiViajes}</h3><p>Viajes Activos</p></div>
        <div className="logi-kpi-card"><h3>{kpiMantenimiento}</h3><p>Alertas Mantenimiento</p></div>
      </div>

      <div className="logi-tabs">
        <button className={tab === "mapa" ? "logi-tab-active" : ""} onClick={() => setTab("mapa")}>
          🗺️ Mapa en Vivo
        </button>
        <button className={tab === "camiones" ? "logi-tab-active" : ""} onClick={() => setTab("camiones")}>Flota</button>
        <button className={tab === "choferes" ? "logi-tab-active" : ""} onClick={() => setTab("choferes")}>Choferes</button>
        <button className={tab === "control" ? "logi-tab-active" : ""} onClick={() => setTab("control")}>Historial Ordenes</button>
        <button className={tab === "operaciones" ? "logi-tab-active" : ""} onClick={() => setTab("operaciones")}>Operaciones</button>
      </div>

      {loading && tab !== "mapa" && <div className="logi-loading"><p>Cargando gestión...</p></div>}
      ...
      <div className="logi-content">
          ...
          <>
            ...
            <div className="logi-table-container">
              <table className="logi-table">
                <thead>
                  <tr>
                    <th>Patente</th><th>Tipo</th><th>Temp (min/max)</th><th>Transportista</th><th>Desinfección</th>
                  </tr>
                </thead>
                <tbody>
        ...
        {tab === "choferes" && (
          <div className="logi-table-container">
            <table className="logi-table">
              <thead>
                <tr>
                  <th>Nombre</th><th>Teléfono</th><th>Licencia</th><th>Vence</th><th>Estado</th>
                </tr>
              </thead>
              <tbody>
                {choferesOrdenados.map(ch => {
                  ...
                  return (
                    <tr key={ch.chofer_id} className={clase}>
                      ...
                      <td>
                         {dias < 0 ? <span className="text-red-600 font-bold">VENCIDA</span> : 
                          dias <= 90 ? <span className="text-orange-500 font-bold">Vence pronto</span> : "OK"}
                      </td>
                    </tr>
                  );
                })}
        ...
        {tab === "control" && (
          <div className="logi-table-container">
            <table className="logi-table">
              <thead>
                <tr><th>Código</th><th>Cliente</th><th>Transportista</th><th>Patente</th><th>Estado</th></tr>
              </thead>
              <tbody>
                {safeArray(ordenes).map(o => (
                  <tr key={o.orden_id}>
                    ...
                    <td>{o.patente}</td>
                    <td><span className="px-2 py-1 rounded bg-gray-200 text-xs">{o.estado}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {tab === "operaciones" && (
          <div className="logi-table-container">
            <table className="logi-table">
              <thead>
                <tr><th>Orden</th><th>Cliente</th><th>Patente</th><th>Estado</th></tr>
              </thead>
              <tbody>
                {operacionesActivas.map(o => (
                  <tr key={o.orden_id}>
                    ...
                    <td>{o.cliente_nombre}</td>
                    <td>{o.patente}</td>
                    <td className="text-green-600 font-bold">{o.estado.toUpperCase()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
