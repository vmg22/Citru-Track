...
export const generarRemitoRechazado = async (datosPDF) => {
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.text(`Chofer: ${choferNombre || "No asignado"}`, 18, yPos + 12);
  doc.text(`DNI: ${choferDni || "N/A"}`, 100, yPos + 12);
  doc.text(`Vehículo: ${camionTipo || "No asignado"}`, 18, yPos + 17);
  doc.text(`Patente: ${camionPatente || "N/A"}`, 100, yPos + 17);
  ...
  doc.text(`Destino: ${destino || "No especificado"}`, 110, yPos + 11);
